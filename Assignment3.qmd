...
dropout %>%
  group_by(Target) %>%
  summarise(across(
    c("Curricular units 1st sem (credited)", "Curricular units 1st sem (evaluations)", "Curricular units 1st sem (grade)"), 
    mean, na.rm = TRUE
  ))%>%
 knitr::kable()
```
...
```{r}
Long_format <- dropout %>%
  select(starts_with("Curricular_units"))
  summarise(across(c("Curricular units 1st sem (credited)", "Curricular units 1st sem (evaluations)", "Curricular units 1st sem (grade)"), mean, na.rm = TRUE)) %>%
